...
const getData = (symbol, startDate, endDate) => {
    return new Promise((resolve, reject) => {
        axios.get('http://localhost:5000/returns/' + symbol + "/" + startDate + "/" + endDate)
            .then(response => {
...
const getStatistics = (symbol, startDate, endDate) => {
    return new Promise((resolve, reject) => {
        axios.get('http://localhost:5000/statistics/' + symbol + "/" + startDate + "/" + endDate)
            .then(response => {
...
class StockReturnChart extends Component {
    ...
    render() {
        const options = getChartOptions(this.handleSelection.bind(this), this.handleClick.bind(this));
